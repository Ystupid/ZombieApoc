...
namespace Cysharp.Threading.Tasks
{
    public static partial class UniTaskExtensions
    {
        ...
        public static void AddCallback(this UniTask task, Action callback)
        {
            AddCallbackInternal(task, callback).Forget();
        }

        private static async UniTaskVoid AddCallbackInternal(UniTask task, Action callback)
        {
            await task;
            callback?.Invoke();
        }

        public static UniTask<T> AddCallback<T>(this UniTask<T> task, Action<T> callback)
        {
            return AddCallbackInternal(task, callback);
        }

        private static async UniTask<T> AddCallbackInternal<T>(UniTask<T> task, Action<T> callback)
        {
        ...
        public static UniTask<T> AddCallback<T>(this UniTask<T> task, Action<T> callback, string key = null, Action<string> failedCallback = null)
        {
            return AddCallbackInternal(task, callback, key, failedCallback);
        }

        private static async UniTask<T> AddCallbackInternal<T>(UniTask<T> task, Action<T> callback, string key = null, Action<string> failedCallback = null)
        {
            T result = default(T);
            try
            {
